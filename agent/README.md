...
The Python trading agent: the loop, the models, the maths and the record it keeps.
For what the project is and how the repository fits together, start at the
[repository README](../README.md). Paper trading only.

**Everything on this page runs from this directory.** The agent owns its own
`pyproject.toml`, virtualenv, `config.yaml`, `.env` and `data/`, so `uv run`
from the repository root finds no project and fails.
...
```bash
cd agent                      # every command below runs from here
...
uv run python tests/scaffold_*.py    # the seven scaffolds, deliberately not collected (D-079)
uv run python scripts/suite_at.py 30 # the suite with the clock advanced 30 days
```
