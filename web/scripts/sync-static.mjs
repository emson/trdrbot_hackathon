...
const FILES = [
	...
	{ src: 'docs/risk_appetite_explorer.html', dest: 'risk-explorer.html', label: 'Risk Appetite Explorer' },
...
<div style="position:sticky;top:0;z-index:999;background:#17242B;color:#F1F4EF;font-family:'IBM Plex Mono',ui-monospace,Menlo,monospace;font-size:12px;padding:8px 16px;display:flex;gap:10px;align-items:center;letter-spacing:.02em">
	<a href="/" style="color:#57C7A7;text-decoration:none;font-weight:600">&larr; trdrbot.com</a>
...
for (const f of FILES) {
	...
	if (/<body[^>]*>/i.test(html)) {
		html = html.replace(/<body([^>]*)>/i, (m) => `${m}${BANNER(f.label)}`);
	} else {
		html = BANNER(f.label) + html;
	}
	fs.mkdirSync(STATIC, { recursive: true });
	fs.writeFileSync(path.join(STATIC, f.dest), html, 'utf8');
	count++;
