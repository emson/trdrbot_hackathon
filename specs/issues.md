...
- **I-29 · The bootstrap base rate is overconfident by 15-18pp where credit spreads live**
  ...
  magnitude. **PARTIALLY ADDRESSED (D-089):** a fitted variance-inflation correction was fitted
  and wired into the muse's base rate, holdout-validated, with `base_inflate` recorded on every
  verdict for the forward audit.
  **RE-OPENED AND RE-MEASURED 2026-09-03 (I-100, D-119).** The harness that produced
  k=1.30/1.30/1.25 drew `round(h * 252/365)` returns and scored them against the close h SESSIONS
  ahead - so most of that k was the unit mismatch, not the tails. Re-fitted on 99 tickers with
  the units corrected: **k = 1.10 / 1.05 / 1.00** at 3/5/10 SESSIONS, and the holdout gains are
  now marginal (Brier 0.2023 -> 0.2016, 0.2142 -> 0.2137, and 10 sessions **withdrawn** at 1.0).
  The 21,280-forecast measurement above was made with the same mismatched harness, so **the
  15-18pp figure is not currently supported and must not be cited until it is re-measured**;
  every in-code citation of it now says so. That is an honest outcome, not a failure: a small
  real correction survives at short horizons and the large one was an artifact of our own ruler.
  **Still open:** (a) root cause - the residual correction is empirical, not explanatory; (b) the
  15-18pp magnitude, pending re-measurement with the corrected harness; (c) **the forward audit**
  - score calibrated base vs raw base as predictors once the pending muse resolutions land; that
  result decides whether the correction extends to the pricing grid or is withdrawn entirely.
