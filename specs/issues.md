...
- **I-127 · `agent/data/metrics.jsonl` carries a row stamped a year ahead of every other timestamp** - OPEN, unexplained. The file's last `snapshot` row reads `ts: "2027-10-08T11:27:29Z"` against every neighbouring row in the 2026-08/09 range, and `model.cal_age_days` on that same row reads 400 - consistent with whatever wrote it having a wrong clock, not with a real gauge value. Found while building notes/028's demo page, which reads this file's `calibration.*` gauges; not yet fixed, because the append-only file itself is sacred (never hand-edited) and the writer that produced the bad row has not been identified. A gauge reader that ignores any row whose `ts` is after export time is the safe mitigation; find the actual writer before trusting `metrics.jsonl` for a time-series chart.
- ~~**I-126 · Every page downloaded the whole 898 KB snapshot before slicing it in the browser**~~ **FIXED 2026-09-03 (D-123).** Every `+page.js`/`+layout.js` was a UNIVERSAL load, so Vite bundled `snapshot.json` into one shared client chunk (864,845 bytes, containing the file's own text) that every page fetched regardless of what it actually rendered - the per-page `load()` narrowing believed to bound page weight bounded nothing. Renamed every loader to `+page.server.js`/`+layout.server.js`; under `adapter-static` + `prerender` these still run at build time, but only each page's own returned slice reaches its HTML. Verified: largest client chunk after is 46,316 bytes and contains no snapshot text; the rendered `<body>` of `index.html`, `ledger.html` and `scoreboard.html` diffed byte-identical before and after.
- **I-125 · Equal arms promote about one time in three at a 50% base rate under the global floors** - OPEN, measured, mitigated per lever (notes/026 section 5). The muse's ~89% survival hid a sequential-peeking cost: `verdict` is evaluated after every trial, so with symmetric headroom a challenger no better than the incumbent clears `P(better) >= 0.90` after the floors 33% of the time (120 simulated experiments; refuted 23%, timeout 45%), and raising `min_runs`/`min_candidates` barely moves it because peeking continues past the floor. `promote_at: 0.95` for `playbook.catalogue` halves it (17%) for a ~20% cost in detection speed on a real 0.50 -> 0.70 improvement. A false promotion swaps in an equally good catalogue and costs churn, which the churn sentinel bounds - so this is a preference for fewer swaps, not a safety bar. **Not fixed with a proper sequential correction** (an alpha-spending boundary, or evaluating only at fixed looks) because no churn has been observed to justify the machinery; build it the day `coach.promotions_total` climbs faster than the outcome stream can explain.
- **I-124 · The live muse challenger `v1` carries a nine-dash scaffolding line** - OPEN, contained. `clean_prompt` matched only the literal ten-dash fence, so a challenger that echoed the mutation prompt's rule line and miscounted (`- - - - - - - - -`) passed cleaning and validation and has run every paired trial since 2026-08-29 with that line at its tail. Harmless to the muse's output (it is trailing prose) and NOT edited in place: editing either arm mid-trial closes the experiment as `operator_override` (I-91), and the trial is at P=0.79 after 13 runs. The cleaner now strips any dash-only edge line (`_DASH_LINE`), so the next mutation cannot repeat it; if `v1` promotes, its text is copied verbatim - hand-strip the line in the state file then (a supported edit), or let the next challenger replace it.
...
- **I-27 · The Coach's gate reward is ASYMMETRIC, measured over 9 real paired runs** (D-088).
  ...
  the gate reward, never replacing it. **Note 2026-09-03 (D-122):** the second lever's reward sits near 50% and has symmetric headroom, so this asymmetry is the muse's, not the protocol's - and the symmetric case has its own cost, I-125.
- **I-28 · The Coach's outcome audit is designed but unbuilt** (D-088, notes/016 phase 3).
  ...
  they land. **Action due:** build the audit once a promoted variant has ~10 resolved entries. **Note 2026-09-03 (D-122):** `playbook_outcome` rows are the first resolved-outcome stream keyed by lever variant, and `playbook.outcome_hit_rate` trends them beside the fast reward from five outcomes on - the audit can be built on the playbook before the muse has resolutions.
...
- **I-16 · Declines are never scored** (D-076). 18 theses simulated, 0 traded, and no record of
  ...
  legs, resolve at its horizon like a traded one. **AMENDED 2026-09-03 (D-122): delivered in the proposed shape for every structure the agent prices.** `simulate_experiments` journals each candidate with its legs, family and band-conditional read (`structures_simulated`), and `playbook.resolve` scores it at the expiry close (`playbook_outcome`) - traded or declined, exactly like a playbook proposal. What remains open is the READING: no gauge yet compares declined outcomes to traded ones, and the first expiries land 2026-09-04.
