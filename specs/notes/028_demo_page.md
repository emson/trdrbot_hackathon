...
> **BUILT 2026-09-03, commits 1-6 the same day.** `/demo` runs for real against the live
> snapshot (20 of 204 cycles, 74.7 KB, verified in the browser light and dark before being
> called done), not just built. Kept as the design record. **Where the build diverged from
> this plan, the build is right** - the rule notes/016, notes/026 and notes/027 all state,
> applied again:
>
> | Plan said | Built instead | Why |
> |---|---|---|
> | `cycles[].act`/`learn`/`remember` duplicate position fields (fill, exit, attribution, pnl) | `/demo`'s own `+page.server.js` also loads `positions[]` (13 rows, already exported); frames look the traded position up by `position_id` | `positions[]` already carries every one of those fields correctly derived - duplicating them into `cycles[]` would be a second place to get fill/attribution wrong, and 13 positions is a small, already-proven-safe addition to one page's own load |
> | `structures_simulated`/`sizing` join a cycle by `decision_ref` OR interval containment, both treated as options of equal standing | `structures_simulated` joins its THESIS by `thesis_entry_id` unconditionally (a direct link stronger than any window, present since the row was first written) - `decision_ref`/interval containment decide which CYCLE a row belongs to, not which claim a candidate priced | the plan's section 2 had already found `thesis_entry_id` but section 8.1's join pseudocode didn't give it priority over the newer, weaker `decision_ref` link for the one thing it answers best |
> | Sense's `market` block sourced generically "from a candidate row" | Sourced specifically, preferring whichever candidate block a cycle's theses produced first; `priced_at`/`sigma` are `null` (rendered `not recorded`) for `structures_simulated` rows, which never carried those fields - only `playbook` rows do | checked against the real row shapes before rendering anything, not assumed from the plan's illustrative JSON |
> | Inbox item Sense rendering for `position`/`manual`/other kinds, shapes guessed in section 8.1 | Rendered from the SIX real payload types on disk: `opportunity`, `news`, `position_review`, `market_pulse`, `prediction_market`, `manual` - none of which matches the plan's guessed `{kind:"position", position_id, underlying, strategy}` shape | every inbox file under `agent/data/inbox/processed/` was read before writing `_sense_item`, and the real shapes (e.g. `position_review`'s `{reason, moves}`, no `underlying` field at all) differed from the plan's guess |
> | `playbook` candidates get a `name` field to render, same as `structures_simulated` | `playbook.run_arm` never names a candidate (only `family` + `legs`) - `_candidate_name` derives one from strikes + family when `name` is absent | found by running the real export and inspecting a playbook row, not assumed from the plan's contract |
> | `funnel.structures.claims_priced` = count of distinct thesis ids seen on `structures_simulated` rows | = count of `structures_simulated` + `playbook` ROWS (9, not 3) | the plan's own definition silently excluded every playbook-priced claim, since only `structures_simulated` carries a `thesis_entry_id` to count by - inconsistent with `kept`/`thrown_out` two lines below it, which already summed candidates across both kinds |
> | `closes` window: "the last 40 daily closes up to the decision day" (section 8.1's illustrative shape reads `dates`/`closes` with no stated bound) | `closes_window()` bounds explicitly at `max(decision_day, horizon)` if the horizon has been reached, else at the decision day - never at today | `returns/<TICKER>.json` holds ONE continuously-updated series, not a snapshot per decision; an unbounded slice of "the last 40" would silently draw an old cycle's chart through sessions it could not have known about yet - caught by re-reading `market_stats.py` before writing the query, not by testing after |
> | The payoff chart's shaded band gets an inline `"the claim"` label (section 4.2) | No inline label - the caption above the chart already names it | a real breakeven landed exactly on a narrow band's edge on the first real cycle tested in the browser (PLTR), colliding with the label at the same y-coordinate; found by looking at the rendered chart, not assumed away |
> | `decision.ts <= row.ts <= outcome.ts` window membership as the primary join, `decision_ref` as a refinement (section 8.1's ordering) | `decision_ref` (or `thesis_entry_id`) checked FIRST and unconditionally when present; interval containment is the fallback for rows that predate those fields, never a competing signal | tested directly: running interval containment on a row that already carries `decision_ref` risks assigning it to a DIFFERENT window than the one it explicitly names, on the exact concurrent-batch shape section 2 validated against |
